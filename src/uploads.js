...
// Separate from UPLOADS_DIR and never served statically (no express.static
// mount points here) — this is where real identity-verification documents
// (government ID photos/PDFs) live. Portfolio photos, job photos, etc. are
// meant to be shown to other users; an ID document is not, so it gets its
// own directory with its own access-controlled route (see GET
// /api/admin/verification/:id/document in admin.routes.js) instead of a
// public URL anyone with the link could open.
...
if (usingConfiguredPrivateDir) {
  console.log(`✅ Private verification documents will be saved to: ${PRIVATE_UPLOADS_DIR} (PRIVATE_UPLOADS_DIR is set)`);
} else {
  console.log(`⚠️  PRIVATE_UPLOADS_DIR is not set — verification documents will be saved to ${PRIVATE_UPLOADS_DIR}, which will NOT survive a redeploy or restart on most hosting platforms. Set PRIVATE_UPLOADS_DIR to a path on your persistent disk (e.g. /var/data/private-uploads on Render) for uploaded ID documents to actually stick around.`);
}
